...
## Summary
- 1216 nodes · 2405 edges · 77 communities (61 shown, 16 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 182 edges (avg confidence: 0.91)
...
### Community 19 - "._start_attack"
Cohesion: 0.20
Nodes (6): find_markers(), Return the template with all ``§`` markers removed (defaults kept)., Split a ``§``-marked template into (clean_text, markers). Markers come in…, strip_markers(), Pitchfork/cluster bomb use one set per position; others use one., Wrap each ``name=value`` value (query + body) in markers.
...
Cohesion: 0.16
Nodes (4): Slot, Public entry point for the container to send this tab's request., One request/response pane with full Repeater controls., RepeaterSession
...
Cohesion: 0.08
Nodes (20): AbstractEventLoop, Pattern, QObject, ResultCallback, _schedule(), HttpResult, Schedule a request; ``callback`` is invoked on the sender thread. Returns a…, A cancellable handle to an in-flight request. The handle wraps the… (+12 more)
...
Cohesion: 0.13
Nodes (11): Live test: RepeaterTab.load_from_record + Send performs a real request., Async HTTP sender for Repeater/Intruder. Runs an ``httpx.AsyncClient`` on a…, Plain data structures passed between the proxy engine, storage, and UI. These…, SQLite persistence for captured flows. Uses the stdlib ``sqlite3`` module…, Custom QAbstractTableModel backing the traffic history view. We use a hand-…, HTTP history: the traffic table plus a request/response detail view. Emits a…, Table model for the Intercept tab's activity log. Each intercepted request…, Intruder tab: a container of independent attack sessions. Each "Send to… (+3 more)
...
Cohesion: 0.11
Nodes (13): finish(), poll_for_ca(), Verify the CA is generated in Bidoytu's confdir and can be exported. Starts the…, on_started(), Verify the capture addon stores DECODED bodies (gzip is undone). Routes a…, Verify starting the proxy on an occupied port produces a clean error signal…, after_start(), finish() (+5 more)

### Community 41 - "BodyStore"
Cohesion: 0.18
Nodes (7): BodyStore, Path, On-disk store for large request/response bodies. Rather than storing large…, Content-addressed file store for body payloads. Files are sharded into…, Write ``data`` to the store and return its relative path. Identical content…, Read body content previously returned by :meth:`store`., Storage layer: SQLite persistence + on-disk body store.
...
### Community 59 - "_NoFocusRectStyle"
Cohesion: 0.60
...
Cohesion: 0.16
Nodes (12): absolute_url(), build_request_text(), build_response_text(), parse_response_text(), ParsedResponse, Helpers for converting between raw HTTP text and structured parts. Used by…, Assemble a raw HTTP response string from parts., Parse raw HTTP response text into a :class:`ParsedResponse`. Tolerant of both… (+4 more)

### Community 78 - "._open_or_recover"
Cohesion: 0.29
...
- **Why does `FlowRecord` connect `FlowRecord` to `InterceptView`, `MessageView`, `HistoryView`, `IntruderSession`, `InterceptActivityModel`, `intruder_session.py`, `RepeaterTab`, `ProxyEngine`, `CaptureAddon`, `MainWindow`, `RepeaterSession`, `AttackRunner`, `IntruderTab`, `main_window.py`, `content_type_from_headers`, `smoke_test.py`, `repeater_session.py`, `QMenu`, `config.py`, `BodyStore`, `FlowTableModel`, `.cookies`, `http_utils.py`?**
  _High betweenness centrality (0.223) - this node is a cross-community bridge._
- **Why does `IntruderSession` connect `IntruderSession` to `QHBoxLayout`, `MessageView`, `HttpHighlighter`, `intruder_session.py`, `IntruderResultsModel`, `FlowRecord`, `PayloadSetEditor`, `._start_attack`, `AttackRunner`, `._restore_attack`, `IntruderTab`, `main_window.py`, `wrap_selection`, `FindBar`, `AsyncHttpSender`?**
  _High betweenness centrality (0.114) - this node is a cross-community bridge._
- **Why does `MessageView` connect `MessageView` to `repeater_session.py`, `QHBoxLayout`, `InterceptView`, `http_utils.py`, `CollaboratorTab`, `IntruderSession`, `HttpHighlighter`, `intruder_session.py`, `MainWindow`, `body_format.py`, `collaborator_tab.py`, `RepeaterSession`, `main_window.py`, `content_type_from_headers`?**
